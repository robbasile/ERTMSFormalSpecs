...
namespace DataDictionary.RuleCheck
{
    ...
    public class RuleCheckerVisitor : Visitor
    {
        ...
        {
            CheckGraphAndSurfaceExpression();
        }
        ...
        ///  - DecelerationProfile
        /// </summary>
        private void CheckGraphAndSurfaceExpression()
        {
            CheckReferences(Dictionary.EFSSystem.DecelerationProfilePredefinedFunction);
            CheckReferences(Dictionary.EFSSystem.FullDecelerationForTargetPredefinedFunction);
        }
        ...
        private void CheckCall(Call call)
        {
            ...
            {
                CheckGraph(call, decelerationFunction.SpeedRestrictions);
                CheckSurface(call, decelerationFunction.DecelerationFactor);
            }
            else if (call.Called.Ref == fullDecelerationForTargetFunction)
            {
                CheckSurface(call, fullDecelerationForTargetFunction.DecelerationFactor);
            }
        }
        ...
        private void CheckGraph(Call call, Parameter parameter)
        {
            ...
            {
                IsValidGraphValue(graphFunctionExpression, parameter);
            }
        }
        ...
        /// <param name="call"></param>
        /// <param name="parameter"></param>
        private void CheckSurface(Call call, Parameter parameter)
        {
        ...
        private void EnsureGraph(Functions.Function function)
        {
            if (function != null)
            {
                if (function.FormalParameters.Count >= 1)
                {
                    EnsureGraph(function, (Parameter) function.FormalParameters[0]);
                }
                else
                {
                    function.AddError(
                        "Graph cannot be computed on the function, since it does not depend on a single parameter");
                }
            }
        }
        ...
        /// </summary>
        /// <param name="function">The functions to verify that it is graph-able</param>
        /// <param name="parameter">The x parameter on which the graph is built</param>
        private void EnsureGraph(Functions.Function function, Parameter parameter)
        {
            ...
            if (range != null && range.getPrecision() == acceptor.PrecisionEnum.aDoublePrecision)
            {
                ...
                {
                    CheckValidGraphCase(cas, parameter);
                }
            }
            else
            {
                if (parameter.Type != EfsSystem.Instance.DoubleType)
                {
                    function.AddError("Parameter " + parameter.Name + " is not a valid range for a graph");
                }
            }
        }
        ...
        /// <param name="parameter"></param>
        private void CheckValidGraphCase(Functions.Case cas, Parameter parameter)
        {
            foreach (Rules.PreCondition preCondition in cas.PreConditions)
            {
                if (!IsValidGraphPrecondition(preCondition.Expression, parameter))
                {
                    preCondition.AddError("Due to this precondition, the graph of this function cannot be computed");
                }
            }

            if (!IsValidGraphValue(cas.Expression, parameter))
            {
                cas.AddError("Due to this value, the graph of this function cannot be computed");                
            }
        }
        ...
        /// <param name="parameter"></param>
        /// <returns></returns>
        private bool IsValidGraphPrecondition(Expression expression, Parameter parameter)
        {
            bool retVal = false;

            if (IsConstant(expression, parameter))
            {
                retVal = true;
            }
            else
            {
                ...
                if (binaryExpression != null)
                {
                    ...
                    {
                        retVal = IsValidGraphPrecondition(binaryExpression.Left, parameter) &&
                                 IsValidGraphPrecondition(binaryExpression.Right, parameter);
                    }
                }
            }
        ...
        /// <param name="parameter"></param>
        /// <returns></returns>
        private bool IsValidGraphValue(Expression expression, Parameter parameter)
        {
            bool retVal = false;
            ...
            if (binaryExpression != null)
            {
                ...
                {
                    retVal = IsValidGraphValue(binaryExpression.Left, parameter) &&
                             IsValidGraphValue(binaryExpression.Right, parameter);
                }
            }
            else
            {
                ...
                {
                    retVal = IsValidGraphValue(reduceExpression.IteratorExpression, parameter) &&
                             IsConstant(reduceExpression.InitialValue, parameter) &&
                             IsConstant(reduceExpression.ListExpression, parameter) &&
                             IsConstant(reduceExpression.Condition, parameter);
                }
                else
                {
                    ...
                    if (call != null)
                    {
                        if (call.Called.Ref is Cast && call.ActualParameters.Count == 1)
                        {
                            Expression casted = call.ActualParameters[0];
                            retVal = IsValidGraphValue(casted, parameter);
                        }
                                 ...
                                 call.Called.Ref == EfsSystem.Instance.AddIncrementPredefinedFunction)
                        {
                            ...
                            retVal = IsValidGraphValue(left, parameter) 
                                && IsValidGraphValue(right, parameter);

                            foreach (Expression callParameter in call.AllParameters)
                            {
                                retVal = retVal && IsConstantOrParameter(callParameter, parameter);
                            }
                        }
                        else
                        {
                            ...
                            if (call.ParameterAssociation != null)
                            {
                                ...
                                {
                                    if (callParameter.Value.Ref == parameter)
                                    {
                                        newParameter = callParameter.Key;
                                    }
                                    else
                                    {
                                        retVal = retVal && IsConstant(callParameter.Value, parameter);
                                    }
                                }
                            }
                            else
                            {
                                retVal = IsValidGraphValue(call.Called, parameter);
                            }
                            ...
                            if (newParameter != null)
                            {
                                ...
                                if (function != null)
                                {
                                    EnsureGraph(function, newParameter);
                                }
                            }
                        }
                    }
                    else
                    {
                        ...
                        if (functionExpression != null)
                        {
                            ...
                            {
                                IsValidGraphValue(functionExpression.Expression, functionExpression.Parameters[0]);
                            }
                        }
                        else
                        {
                            ...
                            if ( derefExpression != null )
                            {
                                ...
                                {
                                    EnsureGraph(derefFunction);
                                    retVal = true;
                                }
                                else
                                {
                                    retVal = IsConstant(expression, parameter);
                                }
                            }
                            else  if (IsConstant(expression, parameter))
                            {
                                retVal = true;
                            }
                        }
        ...
        /// <param name="parameter"></param>
        /// <returns></returns>
        private bool IsConstant(Expression expression, Parameter parameter)
        {
            bool retVal = false;
        ...
        /// <param name="parameter"></param>
        /// <returns></returns>
        private bool IsConstantOrParameter(Expression expression, Parameter parameter)
        {
            bool retVal = IsConstant(expression, parameter);
            ...
            if (! retVal)
            {
                ...
                {
                    retVal = unaryExpression.Term.Ref == parameter;
                }
            }
        ...
        /// </summary>
        /// <param name="surfaceFunction"></param>
        private void EnsureSurface(Function surfaceFunction)
        {
